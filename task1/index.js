...
users.forEach(data => {
    let date = reverseDate(data.registrationDate);
    let dateReversed = new Date(date);
    let ms = dateReversed.getTime();
    if(ms == 1633719600000 || ms == 1633806000000){
       console.log("method 1:", data.firstName)
    }
})
...
users.forEach(data => {
    if(data.registrationDate == "09.10.2021" || data.registrationDate == "10.10.2021"){
        console.log("method 2:", data.firstName)
    }
}) 